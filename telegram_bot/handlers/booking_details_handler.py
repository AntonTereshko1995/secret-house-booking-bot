...
async def show_booking_detail(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not booking.get("is_prepaymented") and not booking.get("is_canceled"):
    ...
    if not booking.get("is_canceled"):
...
async def start_cancel_booking(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Mark as canceled
    try:
        booking = await api_client.update_booking(booking_id, {"is_canceled": True})
    except APIError as e:
        logger.error(f"Failed to update booking: {e}")
        await update.callback_query.edit_message_text("❌ Не удалось отменить бронирование.")
        return END
    ...
    message = (
        ...
        f"Клиент уведомлен: {user.get('contact') if user else 'N/A'}"
    )
    ...
    LoggerService.info(
        ...
        update,
        kwargs={"booking_id": booking_id, "user_contact": user.get("contact") if user else None}
    )
...
async def handle_approve_booking(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if booking.get("is_prepaymented"):
...
async def notify_customer_cancellation(
    ...
    if not user or not user.get("chat_id"):
        LoggerService.warning(
            ...
            kwargs={"booking_id": booking.get("id"), "user_id": user.get("id") if user else None}
        )
        return

    try:
        start_date = datetime.fromisoformat(booking["start_date"])
        tariff_value = booking["tariff"]

        message = (
            f"⚠️ <b>Внимание!</b> ⚠️\n\n"
            f"❌ <b>Ваше бронирование отменено администратором.</b>\n\n"
            f"📋 Детали:\n"
            f"Дата: {start_date.strftime('%d.%m.%Y %H:%M')}\n"
            f"Тариф: {tariff_helper.get_name(Tariff(tariff_value))}\n\n"
        ...
        LoggerService.error(
            ...
            exception=e,
            kwargs={"booking_id": booking.get("id"), "chat_id": user.get("chat_id") if user else None}
        )
...
async def start_change_price(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    context.user_data["manage_user_chat_id"] = user.get("chat_id") if user else None
...
async def handle_price_change_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    api_client = BackendAPIClient()
    try:
        booking = await api_client.update_booking(booking_id, {"price": new_price})
    except APIError as e:
        logger.error(f"Failed to update booking price: {e}")
        await update.message.reply_text("❌ Не удалось изменить стоимость. Попробуйте позже.")
        return END
...
async def handle_prepayment_change_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    api_client = BackendAPIClient()
    try:
        booking = await api_client.update_booking(booking_id, {"prepayment_price": new_prepayment})
    except APIError as e:
        logger.error(f"Failed to update booking prepayment: {e}")
        await update.message.reply_text("❌ Не удалось изменить предоплату. Попробуйте позже.")
        return END
...
async def start_change_tariff(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    context.user_data["manage_booking_id"] = booking_id
    context.user_data["manage_user_chat_id"] = user.get("chat_id") if user else None
    context.user_data["manage_old_value"] = Tariff(booking["tariff"])
    ...
    current_tariff = tariff_helper.get_name(Tariff(booking["tariff"]))
    message = (
...
async def handle_tariff_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Update booking
    try:
        booking = await api_client.update_booking(booking_id, {"tariff": new_tariff.value})
    except APIError as e:
        logger.error(f"Failed to update booking tariff: {e}")
        await update.callback_query.edit_message_text("❌ Не удалось изменить тариф.")
        return END
...
async def notify_customer_tariff_change(
    ...
    try:
        start_date = datetime.fromisoformat(booking["start_date"])
        new_tariff = Tariff(booking["tariff"])
...
async def start_reschedule_booking(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    context.user_data["reschedule_old_start_date"] = start_date
    context.user_data["reschedule_old_end_date"] = end_date
    ...
    try:
        feature_booking = await api_client.get_bookings_by_date_range(
            start_period.isoformat(), end_period.isoformat()
        )
    except APIError as e:
    ...
    message = (
        ...
        f"Текущая дата: {start_date.strftime('%d.%m.%Y %H:%M')} - {end_date.strftime('%d.%m.%Y %H:%M')}\n\n"
...
async def handle_reschedule_start_date(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if selected:
        ...
        tariff_value = Tariff(booking["tariff"])
        if not tariff_helper.is_booking_available(tariff_value, selected_date_obj):
            error_message = (
                "❌ <b>Ошибка!</b>\n\n"
...
async def show_reschedule_start_time(update: Update, context: ContextTypes.DEFAULT_TYPE, booking_id: int) -> int:
    ...
    tariff_value = Tariff(booking["tariff"])
    if tariff_value == Tariff.WORKER or tariff_value == Tariff.INCOGNITA_WORKER:
        message += (
...
async def show_reschedule_finish_time(update: Update, context: ContextTypes.DEFAULT_TYPE, booking_id: int) -> int:
    ...
    try:
        feature_booking = await api_client.get_bookings_by_date_range(
            (finish_date - timedelta(days=2)).isoformat(),
            (finish_date + timedelta(days=2)).isoformat()
        )
    except APIError as e:
        logger.error(f"Failed to get bookings: {e}")
...
async def handle_reschedule_finish_time(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if selected:
        ...
        tariff_value = Tariff(booking["tariff"])
        if (tariff_value == Tariff.WORKER or tariff_value == Tariff.INCOGNITA_WORKER) and \
        ...
        try:
            created_bookings = await api_client.get_bookings_by_date_range(
                start_datetime.isoformat(), finish_datetime.isoformat()
            )
        except APIError as e:
            logger.error(f"Failed to get bookings: {e}")
            created_bookings = []
        is_any_booking = any(b.get("id") != booking["id"] for b in created_bookings)
        if is_any_booking:
            error_message = (
                "❌ <b>Ошибка!</b>\n\n"
        ...
        rental_price = calculation_rate_service.get_by_tariff(tariff_value)
        ...
        booking_duration_hours = max(
            (booking_end - booking_start).total_seconds() / 3600,
...
async def handle_reschedule_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        updated_booking = await api_client.update_booking(
            booking_id,
            {
                "start_date": start_datetime.isoformat(),
                "end_date": finish_datetime.isoformat(),
                "is_date_changed": True,
            }
        )
    except APIError as e:
        logger.error(f"Failed to update booking: {e}")
        await update.callback_query.edit_message_text("❌ Не удалось перенести бронирование.")
        return END
...
async def notify_customer_reschedule(
    ...
    try:
        ...
        message = (
            ...
            f"📋 Детали:\n"
            f"🎯 Тариф: {tariff_helper.get_name(tariff_value)}\n\n"
            f"📅 <b>Старая дата:</b> {old_start_date.strftime('%d.%m.%Y %H:%M')}\n"
            f"📅 <b>Новая дата:</b> {start_date.strftime('%d.%m.%Y %H:%M')} - {end_date.strftime('%d.%m.%Y %H:%M')}\n\n"
            f"По всем вопросам свяжитесь с администратором."
        )
