...
async def choose_booking(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Find booking from API by selected_bookings
        selected_bookings_list = await api_client.get_user_bookings(draft.user_contact)
        ...
        if booking.get("is_date_changed"):
            error_message = (
                "❌ <b>Ошибка!</b>\n\n"
        ...
        redis_service.update_change_booking_field(update, "old_booking_date", start_date)
        redis_service.update_change_booking_field(update, "booking_price", booking["price"])
        redis_service.update_change_booking_field(update, "booking_tariff", booking["tariff"])
        redis_service.update_change_booking_field(update, "booking_has_sauna", booking.get("has_sauna", False))
        redis_service.update_change_booking_field(update, "booking_has_photoshoot", booking.get("has_photoshoot", False))
        redis_service.update_change_booking_field(update, "booking_has_secret_room", booking.get("has_secret_room", False))
        redis_service.update_change_booking_field(update, "booking_has_white_bedroom", booking.get("has_white_bedroom", False))
        redis_service.update_change_booking_field(update, "booking_has_green_bedroom", booking.get("has_green_bedroom", False))
...
async def enter_start_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if selected:
        ...
        if not tariff_helper.is_booking_available(Tariff[booking["tariff"]], selected_date):
            LoggerService.warning(
    ...
    if selected:
        ...
        try:
            ...
            created_bookings = await api_client.get_bookings_by_date_range(
                draft.start_booking_date.isoformat(), finish_booking_date.isoformat()
            )
        except APIError as e:
        ...
        if (
            booking_tariff == Tariff.WORKER or booking_tariff == Tariff.INCOGNITA_WORKER
        ...
        rental_price = calculation_rate_service.get_by_tariff(booking_tariff)
        end_date = datetime.fromisoformat(booking["end_date"])
        start_date_dt = datetime.fromisoformat(booking["start_date"])
        booking_duration_hours = max(
            (end_date - start_date_dt).total_seconds() / 3600,
...
async def confirm_booking(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        new_price = calculation_rate_service.calculate_price_for_date(
            ...
            tariff=booking_tariff,
            duration_hours=duration_booking_hours,
            is_sauna=booking.get("has_sauna", False),
            is_photoshoot=booking.get("has_photoshoot", False),
            is_secret_room=booking.get("has_secret_room", False),
            is_second_room=booking.get("has_green_bedroom", False) or booking.get("has_white_bedroom", False),
        )

        updated_booking = await api_client.update_booking(
            booking["id"],
            {
                "start_date": draft.start_booking_date.isoformat(),
                "end_date": draft.finish_booking_date.isoformat(),
...
async def choose_booking_message(
    ...
    try:
        booking_list = await api_client.get_user_bookings(draft.user_contact)
        selected_bookings = list(
            filter(lambda x: datetime.fromisoformat(x["start_date"]).date() >= date.today(), booking_list)
        )
...
async def start_time_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(available_slots) == 0:
        ...
        booking_tariff = Tariff[booking["tariff"]]
        if booking_tariff == Tariff.WORKER or booking_tariff == Tariff.INCOGNITA_WORKER:
            message += (
...
async def finish_time_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        finish_date = draft.finish_booking_date.date()
        feature_booking = await api_client.get_bookings_by_date_range(
        ...
        start_time = (
            time(0, 0)
            if draft.start_booking_date.date() != finish_date
            else (draft.start_booking_date + timedelta(hours=MIN_BOOKING_HOURS)).time()
        )
        available_slots = date_time_helper.get_free_time_slots(
            feature_booking, finish_date, start_time=start_time
        )
    except APIError as e:
        logger.error(f"Failed to get bookings: {e}")
        available_slots = []
