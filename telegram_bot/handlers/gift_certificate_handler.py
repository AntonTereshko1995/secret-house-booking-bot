...
async def save_gift_information(update: Update):
    """Save gift certificate information from Redis to database via API"""
    draft = redis_service.get_gift_certificate(update)
    code = string_helper.get_generated_code()

    api_client = BackendAPIClient()
    try:
        gift = await api_client.create_gift({
            "user_contact": draft.user_contact,
            "tariff": draft.tariff.value,
            "has_sauna": draft.is_sauna_included,
            "has_secret_room": draft.is_secret_room_included,
            "has_second_bedroom": draft.is_additional_bedroom_included,
            "price": draft.price,
            "code": code,
        })
        return gift
    except APIError as e:
        logger.error(f"Failed to save gift certificate: {e}")
        raise
