...
def create_callback_data(action, year, month, day, prefix: str):
    return (
        ...
        + prefix
        + "_"
        + "_".join([action, str(year), str(month), str(day)])
    )
...
def create_calendar(
    ...
    data_ignore = create_callback_data(
        str(IGNORE), selected_date.year, selected_date.month, 0, callback_prefix
    )
    data_action = create_callback_data(
        str(ACTION), selected_date.year, selected_date.month, 0, callback_prefix
    )
    keyboard = []
    ...
    row.append(
        InlineKeyboardButton(
            f"{date_time_helper.get_month_name(selected_date.month)} {str(selected_date.year)}",
    ...
    for week in my_calendar:
        ...
        for day in week:
            if day == 0:
                ...
                and date(selected_date.year, selected_date.month, day) < min_date
            ):
    ...
    if selected_date.month != min_date.month:
        month_name = date_time_helper.get_month_name(
            (selected_date - relativedelta(months=1)).month
        )
        row.append(
            InlineKeyboardButton(
                f"⬅️ {month_name}",
