    ...
    def __post_init__(self):
        ...
        try:
            ...
            if "time data" in str(e):
                raise ValueError(
                    f"Invalid date format. Expected YYYY-MM-DD, got start_date: {self.start_date}, end_date: {self.end_date}"
                )
            raise
        ...
        if self.start_time is not None:
            try:
                ...
                raise ValueError(
                    f"Invalid start_time format. Expected HH:MM, got: {self.start_time}"
                )
        ...
        if self.end_time is not None:
            try:
                ...
                raise ValueError(
                    f"Invalid end_time format. Expected HH:MM, got: {self.end_time}"
                )

        if self.price_override is not None and self.price_override < 0:
            raise ValueError(
                f"Price override cannot be negative: {self.price_override}"
            )
