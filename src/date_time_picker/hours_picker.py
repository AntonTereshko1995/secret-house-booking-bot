...
def create_callback_data(action, time: time, prefix: str):
    return (
        ...
        + prefix
        + "_"
        + "_".join([action, time.strftime("%H%M") if time else ""])
    )
    ...
    if free_slots is not None:
        ...
        for slot in free_slots:
            ...
            while current_time <= end_time:
                ...
                next_time = (
                    datetime.combine(now.date(), current_time) + timedelta(hours=1)
                ).time()
        ...
        for i in range(0, len(all_time_slots)):
            ...
            callback_data = create_callback_data(
                str(HOURS), all_time_slots[i], callback_prefix
            )
            row.append(InlineKeyboardButton(time_str, callback_data=callback_data))
    ...
    keyboard.append(
        [
            InlineKeyboardButton(
                action_text,
                callback_data=create_callback_data(str(ACTION), None, callback_prefix),
            )
        ]
    )
