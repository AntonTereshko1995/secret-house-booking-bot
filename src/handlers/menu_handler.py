...
from telegram_bot.services import job_service
...
async def _capture_and_store_user_chat_id(update: Update) -> None:
    """Capture and store user's chat_id in the database via API."""
    navigation_service = NavigationService()
    ...
    try:
        ...
        logger.error(f"Failed to store chat_id: {e}")
        LoggerService.error(
            __name__,
