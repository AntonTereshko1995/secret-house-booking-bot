...
async def enter_finish_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if selected:
        ...
        api_client = BackendAPIClient()
        try:
            start_check = booking.start_booking_date - timedelta(hours=CLEANING_HOURS)
            end_check = finish_booking_date + timedelta(hours=CLEANING_HOURS)
            bookings_in_range = await api_client.get_bookings_by_date_range(
                start_check.isoformat(),
                end_check.isoformat()
            )
            is_any_booking = len(bookings_in_range) > 0
        except APIError as e:
            logger.error(f"Failed to check bookings: {e}")
            is_any_booking = False
        if is_any_booking:
            error_message = (
                "❌ <b>Ошибка!</b>\n\n"
...
async def handle_promocode_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Validate promocode via API
    api_client = BackendAPIClient()
    try:
        result = await api_client.validate_promocode(promo_code)
        is_valid = result.get("valid", False)
        message_text = result.get("message", "")
        promo = result.get("promocode", {}) if is_valid else None
        ...
        logger.error(f"Failed to validate promocode: {e}")
        is_valid = False
        message_text = "Ошибка при проверке промокода"
...
async def confirm_pay(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if booking.gift_id:
        api_client = BackendAPIClient()
        try:
            gift = await api_client.get_gift(booking.gift_id)
        except APIError as e:
...
async def start_time_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    api_client = BackendAPIClient()
    try:
        start_range = booking.start_booking_date.date() - timedelta(days=2)
        end_range = booking.start_booking_date.date() + timedelta(days=2)
        feature_booking = await api_client.get_bookings_by_date_range(
            start_range.isoformat(), end_range.isoformat()
        )
    except APIError as e:
        logger.error(f"Failed to get bookings: {e}")
        feature_booking = []
...
async def finish_time_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    api_client = BackendAPIClient()
    try:
        start_range = booking.finish_booking_date.date() - timedelta(days=2)
        end_range = booking.finish_booking_date.date() + timedelta(days=2)
        feature_booking = await api_client.get_bookings_by_date_range(
            start_range.isoformat(), end_range.isoformat()
        )
    except APIError as e:
...
async def init_gift_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    api_client = BackendAPIClient()
    try:
        gift = await api_client.validate_gift_code(update.message.text)
    except APIError as e:
        logger.error(f"Failed to validate gift code: {e}")
        gift = None
    if not gift:
        return await write_code_message(update, context, True)
...
async def save_booking_information(
    ...
    try:
        booking = await api_client.create_booking({
            ...
            "start_date": cache_booking.start_booking_date.isoformat(),
            "end_date": cache_booking.finish_booking_date.isoformat(),
            "tariff": cache_booking.tariff.value if hasattr(cache_booking.tariff, 'value') else cache_booking.tariff,
            "has_photoshoot": cache_booking.is_photoshoot_included,
            "has_sauna": cache_booking.is_sauna_included,
            "has_white_bedroom": cache_booking.is_white_room_included,
            "has_green_bedroom": cache_booking.is_green_room_included,
            "has_secret_room": cache_booking.is_secret_room_included,
            "number_of_guests": cache_booking.number_of_guests,
            "price": cache_booking.price,
            "comment": cache_booking.booking_comment,
            "gift_id": cache_booking.gift_id,
            "promocode_id": getattr(cache_booking, "promocode_id", None),
            "wine_preference": getattr(cache_booking, "wine_preference", None),
            "transfer_address": getattr(cache_booking, "transfer_address", None),
            "chat_id": chat_id,
        })

        if is_cash:
            booking = await api_client.update_booking(booking["id"], {"prepayment": 0})
