...
def get_free_time_slots(
    ...
    if day == now.date():
        ...
        day_start_dt = day0 + timedelta(hours=start_time.hour, minutes=start_time.minute)
...
def _initialize_month_availability(start_date: date, end_date: date, today: date) -> list:
...
def _update_availability_with_bookings(available_days: list, date_bookings: dict, today: date, now: datetime) -> list:
    ...
    for date_key in available_days:
        if date_key in date_bookings:
            ...
            if has_free_slots:
                ...
                _log_day_availability(date_key, has_free_slots, day_bookings, today, now)
        else:
            # Day has no bookings - keep as available
            updated_available_days.append(date_key)
            print(f"Available slots for {date_key.strftime('%d.%m.%Y')}: 00:00-23:59 (no bookings)")
    
    return updated_available_days

def _check_day_availability(date_key: date, day_bookings: list, today: date, now: datetime) -> bool:
...
def _find_free_slots(day_bookings: list, day_start: datetime, day_end: datetime) -> list:
...
def _log_day_availability(date_key: date, has_free_slots: bool, day_bookings: list, today: date, now: datetime):
    ...
    if has_free_slots:
        ...
        slot_info = [f"{start.strftime('%H:%M')}-{end.strftime('%H:%M')}" for start, end in free_slots]
        print(f"Available slots for {date_key.strftime('%d.%m.%Y')}: {', '.join(slot_info)}")
    else:
