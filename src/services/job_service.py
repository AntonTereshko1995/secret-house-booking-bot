...
from services.chat_validation_service import ChatValidationService
...
class JobService:
    ...
    async def send_booking_details(self, context: CallbackContext):
        ...
        bookings = booking_service.get_booking_by_start_date_period(date.today(), tomorrow)
        LoggerService.info(
            __name__,
    ...
    async def send_feeback(self, context: CallbackContext):
        ...
        bookings = booking_service.get_booking_by_finish_date_period(seven_days_ago, today)
        if not bookings:
            LoggerService.info(
                __name__,
        ...
        for booking in bookings:
            try:
                booking_service.update_booking(booking.id, is_done=True)
    ...
    async def cleanup_invalid_chats(self, context: CallbackContext):
        ...
        try:
            ...
            chat_ids = user_service.get_all_user_chat_ids()
            ...
            for invalid_chat_id in results["invalid_ids"]:
                try:
                    if user_service.deactivate_user(invalid_chat_id):
                        deactivated_count += 1
                        LoggerService.info(
                            __name__,
    ...
    async def cleanup_expired_promocodes(self, context: CallbackContext):
        ...
        try:
            count = promocode_repository.deactivate_expired_promocodes()
            if count > 0:
                LoggerService.info(
                    __name__,
